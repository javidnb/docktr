...
<script lang="ts">
	...
	async function fetchData() {
		...
		try {
			...
			if (!usr) dataLoading.set(false);
			if ($session.user) dataLoading.set(false);
		}
	}
	...
	{#if $session.loggedIn && $session?.user?.doctor}
		<div>
			<div class="sidenav pt-5 d-flex flex-column" class:collapsed={isCollapsed}>
				...
				<button on:click={logout} class="btn mt-auto mb-2 d-flex align-items-center gap-2">
...
{:else}
	<div
		...
		style="height: 100dvh; max-height: 100dvh; overflow-y: hidden"
	>
		<img
			src="https://ik.imagekit.io/d2nwsj0ktvh/docktr/logo.png"
			alt="Sehiyye logo"
			style="width: 150px; margin-block: auto"
		/>
